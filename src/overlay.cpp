...
void Overlay::render_control_panel(Simulator* simulator) {
	...
	if (ImGui::Begin("##MainWindow", nullptr, window_flags)) {
		...
		ImGui::SameLine();
		HelpMarker("Show bounding box of the simulation volume");
		...
		if (multiple_photons) {
			ImGui::EndDisabled();
			ImGui::SameLine();
			HelpMarker("Energy labels are disabled when multiple photons are traced to avoid visual clutter");
		}
		else {
			ImGui::SameLine();
		...
		if (simulator) {
			...
			if (simulator->config.num_photons > 1) {
				...
				ImGui::Text("  Scatter Events: %.1f",
							Experimenter::get_scatter_events() / simulator->config.num_photons);
...
void Overlay::render_file_dialog() {
	...
	if (ImGui::Begin(title, &show_file_dialog_, ImGuiWindowFlags_Modal)) {
		...
		if (file_dialog_mode_ == FileDialogMode::LoadConfig) {
			...
			const char* config_files[] = {"config/config1.in",          "config/config2.in",
										  "config/config3.in",          "config/config4.in",
										  "config/multi-photon-100.in", "config/multi-photon-test.in"};
