...
void Mesh::upload_to_gpu() {
	...
	vao_->set_float_attribute(0, 3, sizeof(RenderVertex),
							  reinterpret_cast<const void*>(offsetof(RenderVertex, position)));
...
void Mesh::render() const {
	...
	if (has_indices()) {
		glDrawElements(static_cast<GLenum>(primitive_type_), static_cast<GLsizei>(indices_.size()), GL_UNSIGNED_INT,
					   nullptr);
	}
	else {
