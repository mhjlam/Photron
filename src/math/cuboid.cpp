...
Cuboid::Cuboid(const glm::dvec3& min_pt, const glm::dvec3& max_pt) noexcept :
...
Cuboid Cuboid::union_bounds(const Cuboid& other) const noexcept {
	return Cuboid{glm::min(min_point_, other.min_point_), 
	              glm::max(max_point_, other.max_point_)};
}
...
std::array<glm::dvec3, 8> Cuboid::corners() const noexcept {
	return {{
		{min_point_.x, min_point_.y, min_point_.z},
