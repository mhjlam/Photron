 ...
 * **Usage Pattern:**
 * ```cpp
 * FAST_LOG_PHOTON_EVENT(photon_id, "scatter", position, direction, weight, voxel_coords, medium_id, energy, "Rayleigh
 * scattering"); FAST_LOG_INFO("Simulation completed with " + std::to_string(photon_count) + " photons");
 * ```
 */
class Logger
{
public:
	 ...
	 */
	void log_photon_event(int photon_id,
						  const std::string& event,
						  const glm::dvec3& position,
						  const glm::dvec3& direction,
						  double weight,
						  const glm::ivec3& voxel_coords = glm::ivec3(-1),
						  int medium_id = -1,
						  double energy = 0.0,
	 ...
	 */
	void log_voxel_emittance(int photon_id,
							 const glm::dvec3& position,
							 const glm::dvec3& direction,
							 double weight,
...
#define FAST_LOG_DEBUG(msg)                    \
	...
	while (0)
#define FAST_LOG_WARNING(msg)                    \
	do {                                         \
		if (Config::get().log()) {               \
			Logger::instance().log_warning(msg); \
		}                                        \
	}                                            \
	while (0)
#define FAST_LOG_ERROR(msg)                    \
	do {                                       \
		if (Config::get().log()) {             \
...
#define FAST_LOG_PHOTON_EVENT(id, event, pos, dir, weight, voxel, med, energy, desc)                    \
	do {                                                                                                \
		if (Config::get().log()) {                                                                      \
			Logger::instance().log_photon_event(id, event, pos, dir, weight, voxel, med, energy, desc); \
		}                                                                                               \
	}                                                                                                   \
	while (0)
#else
