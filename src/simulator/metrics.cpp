...
double Metrics::compute_diffusion_distance() const {
	...
	const auto [min_x, max_x] = std::ranges::minmax_element(
		path_vertices_, [](const glm::dvec3& a, const glm::dvec3& b) noexcept { return a.x < b.x; });
	const auto [min_y, max_y] = std::ranges::minmax_element(
		path_vertices_, [](const glm::dvec3& a, const glm::dvec3& b) noexcept { return a.y < b.y; });
	const auto [min_z, max_z] = std::ranges::minmax_element(
		path_vertices_, [](const glm::dvec3& a, const glm::dvec3& b) noexcept { return a.z < b.z; });
...
void Metrics::print_report(const class Simulator& simulator) {
	...
	for (size_t i = 0; i < mediums.size(); ++i) {
		...
		std::cout << "  Scatter events:      " << std::fixed << std::setprecision(0) << metrics.get_scatter_events()
				  << std::endl;
		std::cout << "  Total path length:   " << std::fixed << std::setprecision(6) << metrics.compute_path_length()
				  << std::endl;
		...
		std::cout << "  Total absorption:    " << std::fixed << std::setprecision(6) << energy.total_absorption
				  << std::endl;
		std::cout << "  Total diffusion:     " << std::fixed << std::setprecision(6) << energy.total_diffusion
				  << std::endl;
		std::cout << "    Reflection:        " << std::fixed << std::setprecision(6) << energy.total_reflection
				  << std::endl;
		std::cout << "    Transmission:      " << std::fixed << std::setprecision(6) << energy.total_transmission
				  << std::endl;
		...
		if (energy_data.is_valid) {
			...
			std::cout << "  Total:               " << std::fixed << std::setprecision(1) << percentages.total_percent
					  << "%" << std::endl;
...
Metrics::EnergyConservation Metrics::calculate_energy_conservation(const Simulator& simulator) const {
	...
	result.total_energy =
		result.surface_reflection + result.total_absorption + result.total_reflection + result.total_transmission;
...
Metrics::EnergyConservationPercentages Metrics::calculate_energy_percentages(const Simulator& simulator) const {
	...
	if (energy.surface_refraction > 0) {
		// Use the total accounted energy as baseline (what actually went into the system)
		// This includes surface reflection + all absorption/reflection/transmission
		percentages.baseline_energy = energy.total_energy;
		...
		percentages.total_accounted =
			energy.surface_reflection + energy.total_absorption + energy.total_reflection + energy.total_transmission;
...
void Metrics::write_percentage_line(std::ofstream& ofs, const std::string& label, double percent) const {
	ofs << std::left << std::setw(25) << (label + ":") << std::fixed << std::setprecision(1) << percent << "%"
		<< std::endl;
}

void Metrics::print_percentage_line(const std::string& label, double percent) const {
	std::cout << "  " << std::left << std::setw(18) << (label + ":") << std::fixed << std::setprecision(1) << percent
			  << "%" << std::endl;
}
...
void Metrics::export_simulation_log(const Simulator& simulator, std::ofstream& ofs) const {
	...
	ofs << std::left << std::setw(28) << "Execution time:" << std::fixed << std::setprecision(1) << elapsed_time_ms_
		<< " ms" << std::endl;
	ofs << std::endl << "################################################################" << std::endl << std::endl;

	// General Simulation Statistics
	const auto& config = Config::get();
	ofs << std::left << std::setw(28) << "Volume Grid:" << config.nx() << "x" << config.ny() << "x" << config.nz()
		<< std::endl;
...
void Metrics::export_medium_statistics(const Simulator& simulator, std::ofstream& ofs) const {
	...
	for (size_t i = 0; i < mediums.size(); ++i) {
		...
		ofs << "  Total absorption:       " << std::fixed << std::setprecision(1) << percentages.absorption_percent
			<< "%" << std::endl;
		ofs << "  Surface reflection:     " << std::fixed << std::setprecision(1)
			<< percentages.surface_reflection_percent << "%" << std::endl;
		ofs << "  Surface refraction:     " << std::fixed << std::setprecision(1)
			<< (100.0 - percentages.surface_reflection_percent) << "%" << std::endl;
		ofs << "  Diffuse reflection:     " << std::fixed << std::setprecision(1) << percentages.reflection_percent
			<< "%" << std::endl;
		ofs << "  Diffuse transmission:   " << std::fixed << std::setprecision(1) << percentages.transmission_percent
			<< "%" << std::endl;
	...
	for (const auto& medium : simulator.mediums) {
		...
		for (const auto& voxel : volume) {
			if (voxel && voxel->material != nullptr && voxel->absorption > 0.0) {
				...
				glm::dvec3 world_pos = bounds.min_bounds
									   + glm::dvec3((voxel->ix() + 0.5) * voxel_size,
													(voxel->iy() + 0.5) * voxel_size,
													(voxel->iz() + 0.5) * voxel_size);

				ofs << 0 << "," // MediumID
					<< voxel->ix() << "," << voxel->iy() << "," << voxel->iz() << "," << world_pos.x << ","
					<< world_pos.y << "," << world_pos.z << "," << voxel->absorption << "," << 1.0
					<< "\n";    // Weight (normalized)
			}
		}
	}
...
void Metrics::export_photon_paths_csv(const Simulator& simulator, std::ofstream& ofs) const {
	...
	for (const auto& photon : photons) {
		...
		ofs << photon.id << "," << entrance_pos.x << "," << entrance_pos.y << "," << entrance_pos.z << "," << exit_pos.x
			<< "," << exit_pos.y << "," << exit_pos.z << "," << termination_pos.x << "," << termination_pos.y << ","
			<< termination_pos.z << "," << photon.get_total_path_length() << "," << photon.get_path_scatter_count()
			<< "," << photon.get_total_absorption_deposited() << "," << (photon.exited_medium() ? "TRUE" : "FALSE")
...
void Metrics::export_emittance_data_csv(const Simulator& simulator, std::ofstream& ofs) const {
		...
		for (const auto& voxel : volume) {
			if (voxel && voxel->material != nullptr) {
				...
				for (const auto& [dx, dy, dz] : neighbors) {
					...
					// Check bounds
					if (nx < 0 || ny < 0 || nz < 0 || nx >= static_cast<int>(volume.width())
						|| ny >= static_cast<int>(volume.height()) || nz >= static_cast<int>(volume.depth())) {
						is_surface = true;
						break;
					}
					...
					const Voxel* neighbor =
						volume(static_cast<uint32_t>(nx), static_cast<uint32_t>(ny), static_cast<uint32_t>(nz));
				...
				if (total_voxel_emittance > 0.0) {
					...
					double energy_normalized =
						total_photon_energy > 0.0 ? (voxel->absorption + voxel->total_emittance()) / total_photon_energy
												  : 0.0;

					ofs << 0 << "," // MediumID
						<< voxel->ix() << "," << voxel->iy() << "," << voxel->iz() << "," << world_pos.x << ","
						<< world_pos.y << "," << world_pos.z << "," << (is_surface ? "TRUE" : "FALSE") << ","
						<< "TRUE,"  // HasTissue
						<< voxel->absorption << "," << voxel->emittance << "," << voxel->specular_reflection << ","
						<< voxel->diffuse_transmission << "," << total_voxel_emittance << "," << energy_normalized
						<< "\n";
				}
			}
		}
	...
	Logger::instance().log_photon_event(-1,
										"SUMMARY",
										glm::dvec3(0),
										glm::dvec3(0),
										0.0,
