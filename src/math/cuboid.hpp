...
/**
 * @brief Modern C++20 Axis-Aligned Bounding Box (AABB) implementation
 * @details Optimized cuboid class with C++20 concepts, constexpr support,
...
public:
	...
	[[nodiscard]] bool contains_all(const Range& points) const noexcept 
		...
		return std::ranges::all_of(points, [this](const auto& point) { 
	...
	[[nodiscard]] bool contains_any(const Range& points) const noexcept 
		requires Point3D<std::ranges::range_value_t<Range>> {
		return std::ranges::any_of(points, [this](const auto& point) { 
			return contains(point); 
		});
	}
