...
struct Emitter
{
	...
	explicit constexpr Emitter(
		uint64_t i, const P& p, const V& d, double w, ExitType exit_classification = ExitType::NONE) noexcept :
		id(i), position(static_cast<glm::dvec3>(p)), direction(static_cast<glm::dvec3>(d)), weight(w),
		exit_type(exit_classification) {}
};
...
public:
	...
	[[nodiscard]] inline double eta() const noexcept {
		return (voxel && voxel->material) ? voxel->material->eta() : 0.0;
	}
	[[nodiscard]] inline double mu_a() const noexcept {
		return (voxel && voxel->material) ? voxel->material->mu_a() : 0.0;
	}
	[[nodiscard]] inline double mu_s() const noexcept {
		return (voxel && voxel->material) ? voxel->material->mu_s() : 0.0;
	}
